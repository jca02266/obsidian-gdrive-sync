...
var require_short_unique_id = __commonJS({
  "node_modules/short-unique-id/dist/short-unique-id.js"(exports, module2) {
    ...
    var ShortUniqueId2 = (() => {
      ...
      return __toCommonJS2(index_exports);
    })();
...
var getFile = async (accessToken, fileId, fileName) => {
  try {
...
var getAccessToken = async (refreshToken, refreshAccessTokenURL, showError = false) => {
  var response;
  await axios_default2.post(refreshAccessTokenURL, {
    refreshToken
  }).then((res) => {
    response = res.data;
  }).catch((err) => {
...
var _uid = new import_short_unique_id.default({ length: 6 });
var randomUUID = () => _uid.randomUUID();
    ...
    this.completeAllPendingSyncs = async () => {
      ...
      try {
        ...
        if (err.message.includes("getFilesList") && err.message.includes("404")) {
          ...
          new import_obsidian2.Notice("ERROR: Vault folder not found in Google Drive. Please re-initialize vault from plugin settings.", 8e3);
          return;
        }
...
` + content);
    ...
    this.isNetworkError = (err) => {
      ...
      return msg.includes("UnknownHostException") || msg.includes("Failed to fetch") || msg.includes("No address associated with hostname") || msg.includes("NetworkError");
    };
    this.writeToErrorLogFile = async (log) => {
      ...
      const logBody = networkError ? `${log.name}-${log.message}` : `${log.name}-${log.message}-${log.stack}`;
...
${new Date().toString()}-${logBody}`);
          ...
          try {
            await this.app.vault.create(ERROR_LOG_FILE_NAME, `${new Date().toString()}-${logBody}`);
          } catch (err) {
    ...
    this.initFunction = async () => {
      ...
      try {
        if (res != "error" && res != "network_error") {
          ...
          var folders = await getFoldersList(this.settings.accessToken, "root");
...
var syncSettings = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian2.Setting(containerEl).setName("Reset vault initialization").setDesc("Clear the vault connection so you can re-initialize from scratch. Use this when you want to delete the existing Drive vault and start over. After resetting, delete the vault folder from Google Drive manually, then click 'Initialize vault'.").addButton((button) => {
      button.setButtonText("Reset").setWarning().onClick(async () => {
        ...
        await this.plugin.saveSettings();
        new import_obsidian2.Notice("Vault reset. Please delete the vault folder from Google Drive, then re-initialize.");
        this.display();
      });
    });
    ...
    containerEl.createEl("p", {
      text: "Override these only if you are self-hosting the authentication server. Leave empty / click 'Reset' to use the public default endpoints.",
      cls: "setting-item-description"
    });
    new import_obsidian2.Setting(containerEl).setName("Fetch refresh-token URL").setDesc("The page the user opens in a browser to obtain a Google refresh token (the 'Open this link to log in' link above points to this URL).").addText((text) => text.setPlaceholder("https://your-auth-server/auth/obsidian").setValue(this.plugin.settings.fetchRefreshTokenURL).onChange(async (value) => {
      this.plugin.settings.fetchRefreshTokenURL = value.trim() || DEFAULT_SETTINGS.fetchRefreshTokenURL;
      await this.plugin.saveSettings();
      const linkEl = sync.querySelector("a.sync_text");
      if (linkEl) {
        linkEl.href = this.plugin.settings.fetchRefreshTokenURL;
      }
    })).addExtraButton((btn) => btn.setIcon("reset").setTooltip("Reset to default").onClick(async () => {
      this.plugin.settings.fetchRefreshTokenURL = DEFAULT_SETTINGS.fetchRefreshTokenURL;
      await this.plugin.saveSettings();
      this.display();
    }));
    new import_obsidian2.Setting(containerEl).setName("Refresh access-token URL").setDesc("The server endpoint the plugin calls (with your refresh token) to obtain a fresh Google access token.").addText((text) => text.setPlaceholder("https://your-auth-server/auth/obsidian/refresh-token").setValue(this.plugin.settings.refreshAccessTokenURL).onChange(async (value) => {
      this.plugin.settings.refreshAccessTokenURL = value.trim() || DEFAULT_SETTINGS.refreshAccessTokenURL;
      await this.plugin.saveSettings();
    })).addExtraButton((btn) => btn.setIcon("reset").setTooltip("Reset to default").onClick(async () => {
      this.plugin.settings.refreshAccessTokenURL = DEFAULT_SETTINGS.refreshAccessTokenURL;
      await this.plugin.saveSettings();
      this.display();
    }));
  }
